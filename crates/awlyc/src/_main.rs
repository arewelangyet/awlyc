...
// pub fn deserialize_file<T>(src: &str) -> T {}
...
// // we already know unwrap is safe from previous call to fs::canonicalize
// let entry_file_id = FileId(SmolStr::from(
//     fs::canonicalize(input_file_path).unwrap().to_str().unwrap(),
// ));
// let entry_module = modules.get(&entry_file_id).unwrap();
// }
